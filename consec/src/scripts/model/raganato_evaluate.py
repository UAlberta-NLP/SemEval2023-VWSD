...
def framework_evaluate(
    framework_folder: str, gold_file_path: str, pred_file_path: str
    ...
    p, r, f1 = [
        float(command_output[i].split("=")[-1].strip()[:-1]) for i in range(3)
    ]
    return p, r, f1


def sample_prediction2sense(
    sample: ConsecSample, prediction: int, sense_inventory: SenseInventory
) -> str:
    sample_senses = sense_inventory.get_possible_senses(
        sample.disambiguation_instance.lemma,
        sample.disambiguation_instance.pos,
    )
    sample_definitions = [
        sense_inventory.get_definition(s) for s in sample_senses
    ]
...
def raganato_evaluate(
    ...
    float, float, float, Optional[List[Tuple[str, float, float, float]]]
]:
    ...
    consec_samples = list(
        hydra.utils.instantiate(
            samples_generator, dependency_finder=EmptyDependencyFinder()
        )()
    )
    ...
    with open(f"{reporting_folder}/predictions.gold.key.txt", "w") as f:
        for sample, idx in disambiguated_samples:
            f.write(
                f"{sample.sample_id} {sample_prediction2sense(sample, idx, wordnet_sense_inventory)}\n"
            )

    # write predictions to tmp file
    with open(f"{reporting_folder}/predictions.prob.jsonl", "w") as f:
        for sample, idx in disambiguated_samples:
            prob_output = {}
    ...
    p, r, f1 = framework_evaluate(
        ...
        pred_file_path=f"{reporting_folder}/predictions.gold.key.txt",
    )
...
def main(conf: omegaconf.DictConfig) -> None:
    ...
    module.to(
        torch.device(conf.model.device if conf.model.device != -1 else "cpu")
    )
    module.eval()
    module.freeze()
    module.sense_extractor.evaluation_mode = (
        True  # no loss will be computed even if labels are passed
    )
