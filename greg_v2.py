...
# IT: CUDA_VISIBLE_DEVICES=0 python -i greg_v1.py -d /local/storage/ogezi/v-wsd/test/it-en.test.data.txt -g '' -i /local/storage/ogezi/v-wsd/test/test_images/ --surround '"' --model /home/ogezi/ideas/v-wsd/latest_1674988116_epoch=1 --bn_glosses /local/storage/ogezi/v-wsd/test/simple.it.test.bn.data.txt -r v1.translations/prediction/prediction.it.txt
# FA: CUDA_VISIBLE_DEVICES=0 python -i greg_v1.py -d /local/storage/ogezi/v-wsd/test/fa-en.test.data.txt -g '' -i /local/storage/ogezi/v-wsd/test/test_images/ --surround '"' --model /home/ogezi/ideas/v-wsd/latest_1674988116_epoch=1 --bn_glosses /local/storage/ogezi/v-wsd/test/simple.fa.test.bn.data.txt -r v1.translations/prediction/prediction.fa.txt

# EN: CUDA_VISIBLE_DEVICES=0 python -i greg_v2.py -d /local/storage/ogezi/v-wsd/test/en.test.data.txt -g '' -i /local/storage/ogezi/v-wsd/test/test_images/ --surround '"' --model /home/ogezi/ideas/v-wsd/latest_1674988116_epoch=1 --bn_glosses /local/storage/ogezi/v-wsd/test/en.test.bn.data.txt -bm bert-base-uncased -r v2.lang-specific/prediction/prediction_v2.en.txt
# IT: CUDA_VISIBLE_DEVICES=0 python -i greg_v2.py -d /local/storage/ogezi/v-wsd/test/it.test.data.txt -g '' -i /local/storage/ogezi/v-wsd/test/test_images/ --surround '"' --model clip-italian/clip-italian --bn_glosses /local/storage/ogezi/v-wsd/test/it.test.bn.data.txt -bm dbmdz/bert-base-italian-xxl-uncased -r v2.lang-specific/prediction/prediction.it.txt
# FA: CUDA_VISIBLE_DEVICES=0 python -i greg_v2.py -d /local/storage/ogezi/v-wsd/test/fa.test.data.txt -g '' -i /local/storage/ogezi/v-wsd/test/test_images/ --surround '"' --model SajjadAyoubi/clip-fa-vision --bn_glosses /local/storage/ogezi/v-wsd/test/fa.test.bn.data.txt -bm HooshvareLab/bert-fa-zwnj-base -r v2.lang-specific/prediction/prediction.fa.txt

import argparse
import glob
import os
from time import time
from transformers import CLIPProcessor, CLIPModel, CLIPTokenizer, BertModel, BertTokenizer, VisionTextDualEncoderModel, AutoProcessor, AutoTokenizer, AutoModel, CLIPVisionModel, RobertaModel, CLIPFeatureExtractor
import termcolor
import torch
...
# FA: 'SajjadAyoubi/clip-fa-vision'
# IT: "clip-italian/clip-italian"
parser.add_argument('--bert_model', '-bm', default='bert-base-uncased')
# FA: "HooshvareLab/bert-fa-zwnj-base"
...
parser.add_argument('--bn_glosses')
...
if args.lang in ['en', 'it']:
  model = AutoModel.from_pretrained(args.model, low_cpu_mem_usage=True).to(device)
  processor = AutoProcessor.from_pretrained(args.model)
  tokenizer = AutoTokenizer.from_pretrained(args.model)
  bert_model = AutoModel.from_pretrained(args.bert_model, low_cpu_mem_usage=True).to(device)
  bert_tokenizer = AutoTokenizer.from_pretrained(args.bert_model)
elif args.lang == 'fa':
  assert args.model == 'SajjadAyoubi/clip-fa-vision'
  __txt_model = 'SajjadAyoubi/clip-fa-text'
  image_encoder = CLIPVisionModel.from_pretrained(args.model, low_cpu_mem_usage=True).to(device)
  processor = CLIPFeatureExtractor.from_pretrained(args.model)
  text_encoder = RobertaModel.from_pretrained(__txt_model).to(device)
  tokenizer = AutoTokenizer.from_pretrained(__txt_model)
  bert_model = AutoModel.from_pretrained(args.bert_model, low_cpu_mem_usage=True).to(device)
  bert_tokenizer = AutoTokenizer.from_pretrained(args.bert_model)
  def model(**kwargs):
    t_kwargs = dict(kwargs)
    del t_kwargs['pixel_values']
    i_kwargs = dict(kwargs)
    del i_kwargs['input_ids']
    del i_kwargs['attention_mask']
    return namedtuple('Model', 'text_embeds image_embeds')(
      text_encoder(**t_kwargs).pooler_output,
      image_encoder(**i_kwargs).pooler_output,
    )
...
# TODO: does this traverse farsi as epxected?
  ...
  for i in iter:
    ...
    for inst in instance:
      ...
      if args.lang == 'en':
        ...
        def get_def(word, gloss, exs):
          # g_lower = gloss.lower()
          # article = "Un'" if word.lower()[0] in 'aeiou' else "Un "
          # gloss = ("un'" if g_lower[0] in 'aeiou' else 'un ') + gloss
          # e = '' if not exs else f'; {exs[0]}'
          # return f'{article}{word} è {gloss}{e}'
          return gloss
      elif args.lang == 'fa':
        def get_def(word, gloss, exs):
          return f'{word}: {gloss}'
      ...
      wn_glosses = [s.definition() for s in wn_synsets]
      glosses = [get_def(word, g, []) for g in (bn_glosses[i] if bn_glosses else wn_glosses)]
      words.append(word)
      ...
      if args.lang == 'en':
        ...
        contexts.append("Un " + context.replace(word, f"{args.surround}{word}{args.surround}", 1))
      elif args.lang == 'fa':
        contexts.append(context.replace(word, f"{args.surround}{word}{args.surround}", 1))
    ...
    elif args.lang in ['fa', 'it']:
      img_inputs = processor(images=images, return_tensors="pt").to(device)
      txt_inputs = tokenizer(contexts + glossess_flat, return_tensors="pt", padding=True, truncation=True).to(device)
    ...
    for k in range(len(instance)):
      ...
      if args.lang in ['en', 'fa', 'it']:
        res = sublist_in_list(words_tokens[0], bert_inputs.input_ids[k:k+1][0].tolist())
        if res is not None:
          start, end = res
          ...
          sim_context_gloss_bert = sim_context_gloss
