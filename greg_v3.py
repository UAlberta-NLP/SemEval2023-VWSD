...
# EN: CUDA_VISIBLE_DEVICES=0 python -i greg_v2.py -d /local/storage/ogezi/v-wsd/test/en.test.data.txt -g '' -i /local/storage/ogezi/v-wsd/test/test_images/ --surround '' --model /home/ogezi/ideas/v-wsd/latest_1674988116_epoch=1 -bm bert-base-uncased -r v3.multilingual/prediction/prediction.en.txt
# IT: CUDA_VISIBLE_DEVICES=1 python -i greg_v3.py -d /local/storage/ogezi/v-wsd/test/it.test.data.txt -g '' -i /local/storage/ogezi/v-wsd/test/test_images/ --surround '' -r v3.multilingual/prediction/prediction.it.txt --bn_glosses /local/storage/ogezi/v-wsd/test/it.test.bn.data.txt
# FA: CUDA_VISIBLE_DEVICES=1 python -i greg_v3.py -d /local/storage/ogezi/v-wsd/test/fa.test.data.txt -g '' -i /local/storage/ogezi/v-wsd/test/test_images/ --surround '' -r v3.multilingual/prediction/prediction.fa.txt --bn_glosses /local/storage/ogezi/v-wsd/test/fa.test.bn.data.txt

# Test: CUDA_VISIBLE_DEVICES=0 ipython -i -- greg_v3.py -d /home/ogezi/ideas/v-wsd/semeval-2023-task-1-V-WSD-train-v1/sample/data.100.txt -g  /home/ogezi/ideas/v-wsd/semeval-2023-task-1-V-WSD-train-v1/sample/gold.100.txt -i /home/ogezi/ideas/v-wsd/semeval-2023-task-1-V-WSD-train-v1/train_v1/train_images_v1/ --surround '"' --bn_glosses /home/ogezi/ideas/v-wsd/semeval-2023-task-1-V-WSD-train-v1/sample/bn.data.100.txt
...
parser.add_argument('--model', '-m', default='M-CLIP/XLM-Roberta-Large-Vit-B-16Plus')
parser.add_argument('--bert_model', '-bm', default='bert-base-multilingual-uncased')
...
text_encoder = pt_multilingual_clip.MultilingualCLIP.from_pretrained(args.model)
text_encoder.to(device)

tokenizer = AutoTokenizer.from_pretrained(args.model)
assert args.model == "M-CLIP/XLM-Roberta-Large-Vit-B-16Plus"
image_encoder, _, preprocess = open_clip.create_model_and_transforms("ViT-B-16-plus-240", pretrained="laion400m_e32")
image_encoder.to(device)

@torch.no_grad()
def model(texts: list, images: list):
  pre_and_encode = lambda image: image_encoder.encode_image(preprocess(image).unsqueeze(0).to(device))
  return namedtuple('Model', 'text_embeds image_embeds')(
    text_encoder.forward(texts, tokenizer),
    torch.cat([pre_and_encode(image) for image in images]),
  )
...
with torch.no_grad():
  for i in iter:
    ...
    for inst in instance:
      ...
      if args.lang == 'en':
        def get_def(word, gloss, exs):
          ...
          return gloss # f'{article} {word} is {gloss}{e}'
      elif args.lang == 'it':
        def get_def(word, gloss, exs):
          g_lower = gloss.lower()
    ...
    outputs = model(contexts + glossess_flat, images)
